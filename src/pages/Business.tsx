...
import {
  BusinessForm,
} from '../Formscomponents/BusinessForms';

// Import list components
import { 
  BusinessList, 
} from '../Formscomponents/BusinessForms/lists';
...
const Business: React.FC = () => {
  const [activeModule, setActiveModule] = useState<string | null>('businesses'); // Default to 'businesses' tab
  ...
  const { businesses, loading: businessesLoading, refetch: refetchBusinesses } = useBusinessSelection();

  // API endpoints map for all business-related modules
  const endpointMap: Record<string, string> = {
    businesses: '/.netlify/functions/Business-businesses',
  };
  ...
  const modules: BusinessModule[] = [
    {
      id: 'businesses',
      title: 'Affaires',
      description: 'Gestion des affaires commerciales',
      icon: <Briefcase className="w-6 h-6" />,
      color: 'bg-purple-500',
      apiEndpoint: endpointMap.businesses,
      formComponent: BusinessForm,
      listComponent: BusinessList
    },
  ];

  const otherModules: BusinessModule[] = [];
  ...
  const handleModuleClick = (moduleId: string) => {
    ...
    if (!['businesses'].includes(moduleId) && hasList) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (activeModule === 'businesses') refetchBusinesses();
  ...
  const handleView = (item: any) => {
    // The view functionality is now handled by the BusinessList component
    // This function is kept for compatibility but the actual view modal is in BusinessList
  ...
  const getCurrentLoading = () => {
    switch (activeModule) {
      case 'businesses': return businessesLoading;
  ...
  const currentModule = (getCurrentModule() as BusinessModule | null);

  return (
    <div className="space-y-6">
      <PageHeader
        title="Gestion des Affaires" 
        subtitle="Gérez toutes les opportunités commerciales et leur progression"
      />
