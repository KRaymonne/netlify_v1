...
// Import equipment forms
import {
  EquipmentForm,
  AssignmentForm,
  EquipmentRevisionForm,
  CalibrationForm,
  MaintenanceForm,
  RepairForm,
  EquipmentExpenseForm
} from '../Formscomponents/EquimentsForms';

// Import list components
import { 
  EquipmentList, 
  AssignmentList,
  EquipmentRevisionList,
  CalibrationList,
  MaintenanceList,
  RepairList,
  EquipmentExpenseList
} from '../Formscomponents/EquimentsForms/lists';
...
const Equipment: React.FC = () => {
  ...
  const { equipments, loading: equipmentsLoading, refetch: refetchEquipments } = useEquipmentSelection();
  const { users, loading: usersLoading, refetch: refetchUsers } = useUsersSelection();

  // API endpoints map for all equipment-related modules
  const endpointMap: Record<string, string> = {
    equipment: '/.netlify/functions/equipment-equipment',
    assignments: '/.netlify/functions/equipment-assignments',
    revisions: '/.netlify/functions/equipment-revisions',
    calibrations: '/.netlify/functions/equipment-calibrations',
    maintenances: '/.netlify/functions/equipment-maintenance',
    repairs: '/.netlify/functions/equipment-repairs',
    expenses: '/.netlify/functions/equipment-expenses',
  };
  ...
  const modules: EquipmentModule[] = [
    {
      id: 'equipment',
      title: 'Équipements',
      description: 'Gestion des équipements de l\'entreprise',
      icon: <Cpu className="w-6 h-6" />,
      color: 'bg-purple-500',
      apiEndpoint: endpointMap.equipment,
    ...
    },
    {
      id: 'assignments',
      title: 'Affectations',
      description: 'Affectations d\'équipements aux utilisateurs',
      icon: <Users className="w-6 h-6" />,
      color: 'bg-blue-500',
      apiEndpoint: endpointMap.assignments,
      formComponent: AssignmentForm,
      listComponent: AssignmentList
    }
  ];

  const otherModules: EquipmentModule[] = [
    { id: 'revisions', title: 'Révisions', formComponent: EquipmentRevisionForm, apiEndpoint: endpointMap.revisions, listComponent: EquipmentRevisionList },
    { id: 'calibrations', title: 'Calibrations', formComponent: CalibrationForm, apiEndpoint: endpointMap.calibrations, listComponent: CalibrationList },
    { id: 'maintenances', title: 'Maintenances', formComponent: MaintenanceForm, apiEndpoint: endpointMap.maintenances, listComponent: MaintenanceList },
    { id: 'repairs', title: 'Réparations', formComponent: RepairForm, apiEndpoint: endpointMap.repairs, listComponent: RepairList },
    { id: 'expenses', title: 'Dépenses', formComponent: EquipmentExpenseForm, apiEndpoint: endpointMap.expenses, listComponent: EquipmentExpenseList }
  ];
  ...
  const handleModuleClick = (moduleId: string) => {
    ...
    if (!['equipment', 'assignments'].includes(moduleId) && hasList) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (activeModule === 'equipment') refetchEquipments();
  ...
  const getCurrentData = () => {
    ...
    switch (activeModule) {
      case 'equipment': return applyFilter(equipments || []);
      default: {
  ...
  const getCurrentLoading = () => {
    switch (activeModule) {
      case 'equipment': return equipmentsLoading;
  ...
  const getListPropName = () => {
    switch (activeModule) {
      ...
      case 'maintenances': return 'maintenances';
  ...
  const currentModule = (getCurrentModule() as EquipmentModule | null);

  return (
    <div className="space-y-6">
      <PageHeader
        title="Gestion des Équipements" 
        subtitle="Gérez tous les aspects liés aux équipements de l'entreprise"
      />
